...
print(filename)
alldf_zct = pd.read_pickle(filename_true)
...
ZCT_true,E0_true,VAG_true,SAG_true=ZCT.zct(T_tunnel,index_zct,evals,Vmep_zct,s_list,s,'calc','calc','calc')
...
print("Average initial error (%):", "{:.2f}".format(np.mean(error_init)*100.))
...
#print('ZPE_mean - within chemical accuracy (%)',(1-count_ZPE_mean/ntrials)*100)
#print('ZPE_max - within chemical accuracy (%)',(1-count_ZPE_max/ntrials)*100)    
...
for i in range(len(deltaG_max)):
    if deltaG_max[i]==max_err:
        print('maximum',i,deltaG_max[i])
    elif deltaG_max[i]==-max_err:
        print('maximum',i,deltaG_max[i])
    elif deltaG_max[i]==min_err:
        print('minimum',i,deltaG_max[i])
    elif deltaG_max[i]==-min_err:
        print('minimum',i,deltaG_max[i])
        
print('average',(max_err+min_err)/2)
#ZCT Calculations  
...
for index, row in alldf.iterrows():
    ...
    for i in range(len(np.array(index_zct))):
        if mirror:
            ...
            dict_zct.append({'index_zct':np.array(index_zct)[i],'Vmep':MEP[i],'evals':evals_mirrored[i]})
        
        else:
            dict_zct.append({'index_zct':np.array(index_zct)[i],'Vmep':row['Energies'][i],'evals':row['Xfinal'].T[i]})
    ...
    # use True E0 and VAG
    ZCT_trial,E0_trial,VAG_trial,SAG_trial=ZCT.zct(T_tunnel,index_zct,df_zct['evals'],df_zct['Vmep'],s_list,s,E0_true,VAG_true,SAG_true)
    ...
    #dict_zct.append({'ZCT_true':ZCT_true,'ZCT_trial':ZCT_trial,'ZCT_err':ZCT_err,'E0_true':E0_true,'E0_trial',E0_trial,'VAG_true':VAG_true,'VAG_trial',VAG_trial,'SAG_true':SAG_true,'SAG_trial',SAG_trial})
    #print('Trial #',index,'\nZCT:',ZCT_trial,'\nE0:',E0_trial)
    print('Trial #',index,'\nZCT:',ZCT_trial)

plt.figure(0)
plt.hist(ZCT_err)
plt.xlabel('ZCT Error (%)')
...
new_dict=[]
new_dict.append({'ZCT_true':ZCT_true,'E0_true':E0_true,'VAG_true':VAG_true,'SAG_true':SAG_true,'ZCT_trials':np.array(ZCT_all),'ZCT_err':ZCT_err,'E0_trials':E0_list,'VAG_trials':VAG_list,'SAG_trials':SAG_list})
