...
Tarr = [0.001, 200., 298.15, 500., 1000.] #temperature
...
def free_energy_vtst(E,cm,T,cm_true=None,vmc=False):
    ...
    use_cutoff=True
    for freq in cm:
        if vmc: # If we want the same number of modes included from Xfinal and Xtrue. Deemed deprecated 5/1/2021
            if cm_true[j] !=0 and cm_true[j] >= freq_cutoff and freq!=0:
                frqsi = np.longdouble(freq*c)
                G+=R*T*np.log(1-np.exp(-Hovrkt*frqsi))
            if cm_true[j] < freq_cutoff and use_cutoff: #If less than cutoff, replace with cutoff
            ...
            if freq < freq_cutoff and use_cutoff:#If less than cutoff, replace with cutoff
                frqsi = np.longdouble(freq_cutoff*c)
                G+=R*T*np.log(1-np.exp(-Hovrkt*frqsi))
                #print('ping')
...
def enthalpy_entropy(cm,T): 
    ...
    for freq in cm: 
        if freq!=0:
            ...
            H += frqsi/(expui-1.)
            S += ui/(expui-1) - np.log(1-1./expui)
        else: 
            H += 0.
            S += 0.
    H *= h*Na/(kcaltokJ*1000.)
    H += ZPE_calc(cm)
    S *= R/kcaltokJ
    return H, S
